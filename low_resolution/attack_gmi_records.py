...
def create_parser():
    ...
    parser.add_argument('--exp_name', '-exp', default='stylegan_celeba_vgg_records', help='')
...
def attack_single_id(targetnets, z, G, D, evaluation_model, targets_single_id):
    ...
    if os.path.exists(final_z_unselected_path):
        ...
        for i in range(math.ceil(z.shape[0] / batch_size)):
            ...
            z_batch_optimized, batch_epoch_records = optimization.gmi_optimize_records(i ,z_batch, targets_batch, num_iterations)
        ...
        all_epoch_records.extend(batch_epoch_records)

        ################### demo ###################
        csv_path = f'{id_save_dir}/epoch_records.csv'
        with open(csv_path, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['epoch', 'cosine', 'conf'])
