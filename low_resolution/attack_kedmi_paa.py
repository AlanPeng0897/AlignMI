...
from utils.load_gan import get_DCGAN
...
def create_parser():
    ...
    parser.add_argument('--exp_name', '-exp', default='dcgan_celeba_vgg16_taa', help='')
...
def attack_single_id(targetnets, G, D, evaluation_model, targets_single_id):
    ...
    if os.path.exists(final_z_unselected_path):
        ...
        optimization = Optimization(targetnets, G, D, evaluation_model, fea_mean, fea_logvar, config, 
                                    stylegan=False, save_dir=id_save_dir)
        ...
        for i in range(math.ceil(targets_single_id.shape[0] / batch_size)):
            targets_batch = targets_single_id[i * batch_size:(i + 1) * batch_size].cuda()
            print(
                f'\nOptimizing batch {i + 1} of {math.ceil(targets_single_id.shape[0] / batch_size)} targeting classes {set(targets_batch.cpu().tolist())}.'
            )

            # Run attack iteration
            torch.cuda.empty_cache()

            z_batch_optimized = optimization.kedmi_optimize_paa(i, targets_batch, num_iterations)
...
if __name__ == '__main__':
    ...
    G, D = get_DCGAN(dataset=config.gan["prior"], gan_type=config.improved_flag,
    ...
    for target_id in sorted(list(set(targets.tolist()))):
        ...
        final_z, final_targets = attack_single_id(targetnets, G, D, evaluation_model, targets_single_id)
