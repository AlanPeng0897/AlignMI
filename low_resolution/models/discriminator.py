...
#         self.embedding = nn.Embedding(n_classes, dim*4*4*4)

#         # Updated fc_layer to accommodate the additional embedding
#         self.fc_layer = nn.Linear(dim*4*4*4 + 64, 1)
...
#         # Compute minibatch discrimination output
#         mb_out = self.mbd1(feat)

#         # If y is provided, incorporate it
#         if y is not None:
#             embedded_y = self.embedding(y)  # Embed the labels
#             feat = feat + embedded_y  # Combine the feature and the embedded label

#         # Combine the minibatch discrimination output and features
#         combined = torch.cat([feat, mb_out], dim=1)
#         y = self.fc_layer(combined)
