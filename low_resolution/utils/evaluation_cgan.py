...
def evaluate_results_cgan(evaluation_model, G, batch_size, idx_to_class, final_z, final_targets, trainset, targets_single_id, id_save_dir, save_dir, seed=42):
    ...
    evaluate_facenet = DistanceEvaluation(
        evaluation_model, G, trainset, seed)
    ...
    log_imgs = log_imgs.clamp(0, 1)
    
    min_val, max_val = log_imgs.min(), log_imgs.max()
    log_imgs = (log_imgs - min_val) / (max_val - min_val)
