...
import utils.attack_generator as attack
...
class Classifier(BaseModel):
    ...
    def _build_model(self, architecture, pretrained):
        ...
        if 'resnet' in architecture:
            ...
            # model_state_dict = torch.load('/home/comp/23481501/code/Plug-and-Play-Attacks/results/resnet50_20250223_200300/Classifier_0.8452.pth')['model_state_dict']
            # new_state_dict = {}
        ...
        # Note: inception_v3 expects input tensors with a size of N x 3 x 299 x 299, aux_logits are used per default
            ...
            # model_state_dict = torch.load('./results/inceptionv3_celeba/Classifier_0.9487.pth')['model_state_dict']
            # new_state_dict = {}
            # # for key in model_state_dict:
            # #     new_key = key.replace('model._orig_mod.', 'model.')
            # #     new_state_dict[new_key] = model_state_dict[key]
        ...
        elif 'maxviit' in architecture:
            if architecture == 'maxviit':
                model = timm.create_model('maxvit_tiny_tf_224', 
                                          pretrained=pretrained)
            

            if self.num_classes != model.head.fc.out_features:
                # exchange the last layer to match the desired numbers of classes
                model.head = nn.Linear(model.head.fc.in_features,
                                       self.num_classes)
            return model
    ...
    def adv_fit(self,
        ...
        evalloader = DataLoader(validation_data,
                                batch_size=batch_size,
                                shuffle=False,
                                num_workers=dataloader_num_workers,
                                pin_memory=True)
        ...
        for epoch in range(num_epochs):
            ...
            for inputs, labels in tqdm(trainloader,
                ...
                x_adv, _ = attack.PGD(self.model, inputs, labels,
                                      adv_cfg['eps'], adv_cfg['sts'],
                                      adv_cfg['num_steps'], loss_fn=adv_cfg['loss_fn'],
                                      category=adv_cfg['category'], rand_init=True)
                self.train()

                optimizer.zero_grad()
                outputs = self.forward(x_adv)
                nat_outputs = self.forward(inputs)
                ...
                if isinstance(nat_outputs, inception.InceptionOutputs):
                    if self.model.aux_logits:
                        nat_outputs, aux_logits = nat_outputs
                ...
                if adv_cfg['train_method'] == 'AT':
                    ...
                    loss = attack.TRADES_loss(outputs, nat_outputs, labels, beta=6.0)
                elif adv_cfg['train_method'] == 'MART':
                    loss = attack.MART_loss(outputs, nat_outputs, labels, beta=6.0)
                ...
                running_total_loss += loss * num_samples
                metric_train.update(nat_outputs, labels)
