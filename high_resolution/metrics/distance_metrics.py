...
class DistanceEvaluation():
    ...
    def compute_dist(self, w, targets, batch_size=64, rtpt=None):
        ...
        for step, target in enumerate(target_values):
            ...
            distances = torch.cdist(attack_embeddings, target_embeddings, p=2).cpu()
            # distances = - F.cosine_similarity(attack_embeddings.unsqueeze(1), target_embeddings.unsqueeze(0), dim=-1).cpu()
