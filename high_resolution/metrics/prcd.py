...
class PRCD:
    ...
    def compute_metric(self, target_id_list, k=3, rtpt=None):
        precision_list = []
        recall_list = []
        density_list = []
        coverage_list = []
        # for step, cls in enumerate(range(num_classes)):
        for step, cls in enumerate(target_id_list):
            with torch.no_grad():
                ...
                if rtpt:
                    rtpt.step(
                        subtitle=f'PRCD Computation step {step} of {len(target_id_list)}')
