...
from criteria import mmd_loss
from criteria import ct_loss
...
def mmd(G,
    ...
    start_w = initial_w if initial_w is not None else start_w

    target_images = pp_data.to(device).to(torch.float32)
    target_features = facenet(target_images)
    ...
    for step in tqdm(range(num_steps)):
        ...
        for w_batch in DataLoader(TensorDataset(ws), batch_size=1, shuffle=False):
            # 生成合成图像
            batch_synth_images = G.synthesis(w_batch[0], noise_mode='const', force_fp32=True)

            # 使用Discriminator评估合成图像
            discriminator_logits = discriminator(batch_synth_images, None)
            discriminator_loss = nn.functional.softplus(-discriminator_logits).mean()
            total_discriminator_loss += discriminator_loss * w_batch[0].size(0)

            batch_synth_images = torchvision.transforms.functional.center_crop(batch_synth_images, (800, 800))
            batch_synth_images = torchvision.transforms.functional.resize(batch_synth_images, 160, antialias=True)
            synth_images.append(batch_synth_images)

        # 如果你有多个批次，你可能想要计算平均 Discriminator 损失
        discriminator_loss = total_discriminator_loss / len(ws)
        synth_images = torch.cat(synth_images, dim=0)

        synth_features = facenet(synth_images)

        # 2 - MMD loss (synth_features - target features)
        MMD_loss = mmd_loss.mmd(synth_features, target_features)
        # MMD_loss = (synth_features - target_features).mean()
        # MMD_loss = (synth_images - target_images).mean()
        ...
        if step == 0 or (step + 1) % 10 == 0:
            print(
                f'step {step + 1}, loss:{loss.detach().cpu():.4f}, '
        ...
        loss.backward()
        optimizer.step()
        logprint(f'step {step + 1:>4d}/{num_steps}: MMD_loss {MMD_loss:<4.2f} loss {float(loss):<5.2f}')
